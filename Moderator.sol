...
contract Moderator is IModerator, ERC721Metadata,Ownable {
    ...
    constructor() public  ERC721Metadata("Moderator as NFT", "Mod"){
    }
    ...
    function setEscrow(address payable _escrow) public onlyOwner {
        ...
        require(EscrowContract.getModAddress()==address(this),'Wrong escrow contract address');
    ...
    function mint() public onlyOwner {
        ...
        require(tokenId <= maxSupply, 'Moderators supply reach the max limit!');
    ...
    function getModOwner(uint256 modId) external returns(address) {
        require(modId <= totalSupply(),'Illegal moderator ID!');
    ...
    function increaseScore(uint256 modId, uint256 inScore) external returns(bool) {
        ...
        require(escrowAddress == msg.sender,'Only escrow contract can call this method');
    ...
    function decreaseScore(uint256 modId, uint256 deScore) external returns(bool) {
        ...
        require(modScore[modId] >= deScore, 'The score of the mode must be bigger than deScore');
        //increase moderate score
